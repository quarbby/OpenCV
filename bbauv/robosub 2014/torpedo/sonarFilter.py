...
class SonarFilter():
    ...
    def checkDetections(self, grayImg):
        ...
        if len(self.allBoxList) > 0 and self.numFrames == 0:
            for boxMap in self.allBoxList:
                box = boxMap[0]
                cv2.drawContours(grayImg, [box], 0, (0,255,0), 2)
                grayImg = lself.getRangeBearing(grayImg, boxMap[1], boxMap[2], boxMap[3])
            self.allBoxList = []
        return grayImg
    ...
    def checkFrequency(self, box):
        ...
        for box in self.collectedBoxList:
            ...
            if math.sqrt(math.pow(mpoint[0]-midpoint[0], 2) + math.pow(mpoint[1]-midpoint[1], 2)) < self.minBoxDistance :
                freqCount += 1
        if freqCount >= len(self.collectedBoxList)/4:
            return True
        return False    
