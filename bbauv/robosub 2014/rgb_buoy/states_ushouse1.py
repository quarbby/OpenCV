...
class bangBuoy(smach.State):
    ...
    # deltaYMult = 1.3
    deltaYMult = 1.8    # For small pool
    # deltaYMult = 0.8    # For US House
    area = 8800
    count = 0
    # forward_setpoint = 0.50
    forward_setpoint = 0.25     # For small pool
    ...
    def execute(self, userdata):
        ...
        if self.comms.rectArea < 5000:
            ...
            self.forward_setpoint = 0.5     # For small pool
        else:
            # self.forward_setpoint = 0.5
            self.forward_setpoint = 0.3     # For small pool
...
class Centering (smach.State):
    ...
    forward_setpoint = 0.10
    
    # bigArea = 12000
    bigArea = 11000     # US small pool
    changeMultArea = 8500
    ...
    def execute(self, userdata):
        ...
        if self.comms.rectArea > self.bigArea:
            ...
            if self.count > 2:
                self.comms.sendMovement(forward=2.0, timeout=4, blocking=True)   # Shoot forward
                ...
                self.comms.sendMovement(forward=-0.5, depth=self.comms.defaultDepth-0.5,
                            timeout=3, blocking=True)  # Reverse a bit
                self.comms.isAborted = True
