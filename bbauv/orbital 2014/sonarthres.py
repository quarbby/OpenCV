...
class Sonar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.initSub()
        self.sonarFilter = SonarFilter(self.testing)
        self.initUI()
    ...
    def initUI(self):
        ...
        thres_layout, self.thres_slider, self.thres_l = self.make_qslider_box("Thresh", 0, 1000, self.params['thres'])
        thres_offset_layout, self.thres_offset_slider, self.thresOffset_l = self.make_qslider_box("Thres Offset", 0, 1000, self.params['thresOffset'])
        thres_bound_layout, self.thres_bound_slider, self.thresBound_l = self.make_qslider_box("Thres Bound", 0, 1000, self.params['threshBound'])
        ...
        slider_layout.addLayout(thres_bound_layout)
        ...
        self.thres_bound_slider.valueChanged.connect(self.thresBoundChanged)
        ...
        processed_layout.addLayout(filter_layout)
        processed_layout.addLayout(thres_layout)

        # Main frame layout
        self.main_layout.addLayout(slider_layout)
        self.main_layout.addLayout(params_layout)
        self.main_layout.addLayout(image_layout)
        self.main_layout.addLayout(processed_layout)
    ...
    def initSub(self):
        ...
        self.filter_sub = rospy.Subscriber(rospy.get_param('~filter', 
            "/Vision/sonar_filter"), Image, self.filter_callback)
        self.testing = rospy.get_param('~testing', True)
    ...
    def on_timer(self):
        ...
        if filterImg is not None:
            filterImg = self.updateFrontCamImage(filterImg)
    ...
    def thresOffsetSliderChanged(self):
        ...
        self.thresOffset_l.setText(str(self.params['thresOffset']))

    def thresBoundChanged(self):
        self.params['thresBound'] = self.thres_bound_slider.value()
        self.thresBound_l.setText(str(self.params['thresBound']))
